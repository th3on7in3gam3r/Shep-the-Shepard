...
export function ShepAvatar({
  ...
  return (
    <div
      className={cn(
        "relative shrink-0 rounded-full bg-zinc-200/60 p-1 ring-2 ring-zinc-400/50 dark:bg-zinc-700/50 dark:ring-zinc-500/40",
        sizes[size],
    ...
    >
      ...
      {effectiveMood === "listening" && (
        <span className="absolute -right-0.5 -top-0.5 flex size-3">
          <span className="absolute inline-flex size-full animate-ping rounded-full bg-[#00ffc6] opacity-60" />
          <span className="relative inline-flex size-3 rounded-full bg-[#00ffc6]" />
        </span>
      )}
    </div>
  );
