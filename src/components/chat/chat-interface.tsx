...
import { ShepScene } from "@/components/chat/shep-scene";
...
export function ChatInterface() {
  ...
  const displayLine = isClient
    ? (holding || isListening ? liveTranscript || interimTranscript : "") ||
      streamingText ||
      (lastUser && !isStreaming && !isLoading ? getMessageText(lastUser) : "") ||
      (messages.length === 0
  ...
  return (
    <div className="flex h-[calc(100dvh-6.5rem)] flex-col">
      {/* 2D Shep — active cartoon centerpiece */}
      <div className="relative shrink-0 px-0.5">
        <ShepScene
          mood={mood}
          welcomeEntrance={welcomeMood && messages.length === 0}
          className="h-80 w-full rounded-2xl border border-shepherd-sage/30 shadow-[0_8px_32px_-8px_rgba(74,100,68,0.22),0_2px_12px_-4px_rgba(74,100,68,0.1)] ring-1 ring-white/50 sm:h-80"
        />
        <div className="absolute bottom-2 left-0 right-0 px-3 text-center">
          <p
            className="truncate text-xs font-medium text-shepherd-sage"
            suppressHydrationWarning
          >
            {getStatusText()}
          </p>
      ...
      {isClient && chatLive === false && (
        <p className="px-1 text-xs text-muted-foreground">
          ...
          <code className="rounded bg-muted px-1 py-0.5 text-[11px]">.env.local</code> for live
          GPT replies, then restart{" "}
      ...
      <div className="mt-2 shrink-0 space-y-3 border-t border-border/60 pt-3">
        <ChatContextBanner />
